...
local function threshold()
  local name = require("chronicle.config").options.log_level
...
function M.at(level, msg)
  if not require("chronicle.config").options.notify then
    return
  end
  if level < threshold() then
    return
  end
  vim.notify(msg, level, { title = "chronicle" })
end
...
---User-initiated feedback (from a `:Chronicle*` command). Always shown.
---@param msg string
---@param level? integer
function M.echo(msg, level)
  vim.notify(msg, level or vim.log.levels.INFO, { title = "chronicle" })
end
